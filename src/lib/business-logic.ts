...
const ROLE_PERMISSIONS: Record<string, string[]> = {
    ADMIN: ['*'],
    // Bug #9: Match auth.ts — STORE_MANAGER gets everything except 'users'
    STORE_MANAGER: ['dashboard', 'products', 'orders', 'customers', 'partners', 'commissions', 'payouts', 'automation', 'ai', 'analytics', 'fraud', 'shipping', 'returns', 'warehouse', 'reviews', 'support'],
    // STAFF uses explicit per-user permissions only
    STAFF: [],
};
